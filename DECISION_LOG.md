...
## 2026-09-22 -- CHN-17: the scheduler is now confirmed genuinely live, not just built -- and the README's own status table was stale by two days

**Context.** A row-by-row audit of every WBS item (SPN-01 through CHN-33) against the actual repo marked CHN-17 "Partial," on the grounds that `ps aux`, a search for scheduler entrypoint scripts, and recent git log showed no evidence of a running scheduler process. That check was incomplete: it searched filenames for "sched" and missed `scripts/live_runner_p1_agent_test.py` and `scripts/live_runner_teams_agent_test.py`, which had already existed since 2026-09-21 and had substantial DECISION_LOG history of their own (a 31-script `sys.path` fix batch, live_runner-specific bug fixes) that simply hadn't been read yet in this pass.

**Finding.** The project owner pasted a real terminal transcript covering roughly 17:19 to 22:01 on 2026-09-22: both live_runner scripts running unattended, each wiring a real `apscheduler.schedulers.blocking.BlockingScheduler` (or equivalent) with an `IntervalTrigger` (5-minute Graph ingest polling) and a `CronTrigger` (daily digest/nudge/escalation check at each channel's own configured local time -- 17:30 Asia/Colombo for `p1-agent-test`, 13:00 America/New_York for a second real channel, `Teams-agent-test`, not previously covered by this audit). The transcript includes a genuine APScheduler-generated "Run time of job ... was missed by 0:01:07.650814" drift message -- not something producible by hand-editing a log -- plus real nudge (`awaiting_approval`) and escalation (`below_threshold`, `awaiting_nudge`) evaluations firing on their own, and real Supabase syncs after each cycle. This is live, unattended, multi-hour operation against two real Teams channels, satisfying CHN-17's own acceptance bar more completely than the scripted clock-override test it originally asked for.

**Fix.** Corrected the audit verdict for CHN-17/C7 from Partial to Done, and updated three places in README.md that all said essentially "no standalone always-on process yet" (the top Status line, the Quick start section's `make run` explanation, and the C7 row of the status table itself) to instead point at the two live_runner scripts and this evidence, per CHN-30's own rule that the status table must be verifiable by opening a named file.

**Judgment calls.** Did not mark C7 as live-verified purely because a process was running at some point -- confirmed the transcript's own internal evidence (the missed-run drift message, the real per-channel local-time schedules, the real Supabase row counts changing between cycles) rather than taking "it's running" at face value, consistent with how every other live claim in this project has been checked. Noted, rather than glossed over, that neither process is actually running at the moment this entry was written (`ps aux` showed nothing) -- CHN-17 being Done describes the capability, not a claim that it is continuously running right now.

**Non-vacuousness.** Not a code change, so no disable/re-enable cycle applies here; the check that mattered was verifying the transcript's drift message and schedule times could not have come from anywhere except a real running APScheduler instance, and cross-checking the two script paths actually exist and are committed (`git log --oneline -1` against both files).

**Verified.** `scripts/live_runner_p1_agent_test.py` and `scripts/live_runner_teams_agent_test.py` both exist, are committed, and both wire real `IntervalTrigger`/`CronTrigger` instances (`grep` confirmed `add_job` calls against both). README.md's Status line, Quick start note, and C7 row all updated to match.

**Not done, on purpose.** Did not restart either live_runner process from here -- neither is running right now, and doing so is the project owner's own call, not something to do unprompted mid-audit. The separate, still-open CHN-25 gap (no real Copilot Studio agent or Dataverse table yet) is untouched by this entry.

## 2026-09-23 -- Dataverse re-opened as a required CHN-02/CHN-25 component (reverses 2026-09-20), and a YAML re-sync no longer overwrites owner config edits

**Context.** The 2026-09-20 entries closed the Dataverse question with "no table built", mainly because a Dataverse table would keep "a second copy" of the roster data already in SQLite `channel_config`. Re-reading the WBS against that reasoning: Dataverse is a primary tool in two P1 rows (CHN-02 "Python (YAML schema) + Dataverse surface"; CHN-25 "Copilot Studio + Dataverse", "a per-channel config surface backed by Dataverse"), and SPN-04 says outright that "Dataverse is added later as a human-facing config surface, not the system of record". The plan always intended two stores. The duplication argument was against a design the plan had already accepted, so dropping Dataverse was a gap caused by the permissions blocker, not a design improvement.

**Decision.** Dataverse is back in scope for CHN-02/CHN-25, status **Blocked (admin permission)**, not descoped. Design is the "Option B" mirror already implemented in `scripts/sync_from_dataverse.py` (2026-09-21, untested): owners edit roster / update window / exceptions in a Dataverse table from Teams via Copilot Studio; each run pulls those fields into `channel_config` through `ChannelConfigStore.update_channel_config()` (so every change is still audited through the one write path); the pipeline keeps reading SQLite only. The Streamlit editor remains the working fallback until the table exists. README status for CHN-02/CHN-25 should read Partial with this reason.

**Bug found while checking this, fixed.** `ChannelConfigStore.sync_to_db()` upserted every YAML field with `ON CONFLICT ... DO UPDATE`, including the three owner-editable ones, and `run_daily.py` plus every live runner calls it on every run. So any live roster/window/exceptions edit silently reverted to YAML at the next job -- e.g. a member put on the exceptions list would be reported as a non-responder again the next morning. Live evidence: every row in `data/p1_live.db`'s `channel_config` had `updated_at = 2026-09-23 06:09:01` and `version = 1`. The same overwrite would have undone every Dataverse pull, so it had to be fixed before Dataverse could work at all.

**Fix.** `src/p1/config/loader.py`: new `OWNER_EDITABLE_FIELDS`; `sync_to_db()` now seeds those fields for a new channel only and leaves them (and `version`) alone on an existing row, while still re-applying every YAML-only field. `sync_to_db(reset_owner_fields=True)` is the explicit override: YAML wins, `version` is bumped, and one `channel_config.reset_from_yaml` audit row is written per channel. `scripts/sync_channel_config_to_db.py` gained `--reset` for it.

**Verified.** Three new tests in `tests/unit/test_channel_config.py`: a live edit survives a re-sync; YAML-only fields (e.g. `nudge_enabled`, `daily_digest_time`) still apply on re-sync; `reset_owner_fields=True` restores YAML, bumps version and leaves the audit trail. `uv run pytest tests/unit -q`: 484 passed, 2 failed -- both in `test_run_daily_full_flow.py`, caused by the uncommitted `config/channels/demo-alpha.yaml` / `demo-beta.yaml` adding two allowlisted channels that test's expected set doesn't include; unrelated to this change.

**Not done, on purpose.** The spine's generic `ConfigStore.sync_to_db()` (`packages/spine/src/spine/config/store.py`) still resets live edits by design, with a test asserting it; P1 doesn't use it, but P2/P3 will, so it needs the same decision before they do. The admin request (System Customizer for the maker account; a Dataverse application user + client secret for the agent's app registration; `DATAVERSE_URL` in `.env`) has been drafted, not sent -- sending it is the project owner's call.

## 2026-09-23 -- Daily digest and nudge/escalation cron moved off the laptop, onto GitHub Actions (both live channels)

**Context.** `scripts/live_runner_p1_agent_test.py` and `scripts/live_runner_teams_agent_test.py` both stayed running on a personal laptop as a single long-lived process (`BlockingScheduler`, misfire_grace_time defaulting to APScheduler's own 1 second, unset anywhere in this repo). The laptop went idle for ~2h25m spanning p1-agent-test's 17:30 Asia/Colombo digest and nudge/escalation cron on 2026-09-23; APScheduler logged both as "missed" and advanced straight to the next day's occurrence rather than running late, so no digest reached the channel that day. Two structural problems, not one: (a) a laptop that sleeps cannot run an always-on scheduler reliably, and (b) even on a machine that never sleeps, a 1-second grace time turns any brief stall into a fully skipped day.

**Decision, scoped deliberately.** Move only the once-a-day jobs (digest, nudge/escalation) to a GitHub Actions scheduled workflow, per channel. The 5-minute ingest poll stays wherever it already runs (laptop, or later a small always-on VM) -- GitHub Actions cron isn't built for 5-minute granularity (schedules that frequent risk being silently delayed under GitHub's own load, and burn through free-tier Actions minutes fast at ~288 runs/day/channel), and the reliability complaint was specifically about the daily digest, not ingest freshness. This does not touch the misfire_grace_time question for the laptop-based scheduler at all; that's still open if the laptop path is kept for ingest.

**What was built.**
- `.github/workflows/daily-p1-agent-test.yml`, `.github/workflows/daily-teams-agent-test.yml` -- one `schedule:` cron each (12:00 UTC / 17:00 UTC, matching each channel's own local `daily_digest_time`), plus `workflow_dispatch` for manual testing. Each job: restore `data/p1_live.db` + `data/graph_token_cache.bin` from `actions/cache` (keyed per-run with a prefix `restore-keys` fallback, the standard "ever-growing state via cache" pattern), seed the token cache from a secret on a cold start, refresh the Graph token, run that channel's one-shot live pipeline script, then its nudge script.
- `scripts/ci_refresh_graph_token.py` -- new. A thin wrapper around `p1.adapters.graph_auth.get_access_token(allow_interactive=False)`, a function that already existed for exactly this unattended shape (see that module's own docstring -- CHN-01's "separate future step," finally used here rather than reimplemented). Writes the refreshed token to `$GITHUB_ENV` and explicitly registers it with GitHub's `::add-mask::` log masking first, since a runtime-minted token isn't covered by GitHub's automatic masking of `secrets.*` values.
- `scripts/run_live_nudge_teams_agent_test.py` -- new. No one-shot nudge script existed for this channel; `live_runner_teams_agent_test.py` called `run_nudge_job()` directly from inside its own persistent process, which a one-shot GitHub Actions job has no equivalent of. Mirrors `run_live_nudge_p1_agent_test.py` exactly, pointed at Teams-agent-test's channel_id.

**Real limitations, stated plainly, not glossed over.**
1. **Graph auth still needs a human once.** The very first run needs a `data/graph_token_cache.bin` seeded by running `scripts/graph_seed_token_cache.py` locally (real interactive sign-in) and uploading it, base64-encoded, as the `GRAPH_TOKEN_CACHE_B64` repo secret. After that the workflow keeps it refreshed on its own -- but if the cached refresh token is ever revoked or goes unused past Azure AD's own expiry (90 days, or sooner on a password change or an admin action), the next run fails with a clear `GraphAuthError` and the same manual reseed step has to be repeated. This is not solved here; it's the accepted cost of delegated auth in an unattended pipeline (see CHN-01's own Option A vs B tradeoff).
2. **State persistence rides on `actions/cache`, not a durable store.** GitHub evicts cache entries unused for 7 days or once a repo's total cache exceeds 10GB. A single small SQLite file run daily is very unlikely to hit either limit, but this is not the same durability guarantee as a real database -- worth hardening later (e.g. pointing these jobs at Supabase as the actual system of record for a live run) if this needs to be bulletproof rather than "good enough for a pilot."
3. **DST drift on Teams-agent-test.** Its cron is fixed at 17:00 UTC for 13:00 America/New_York (EDT); across the Nov/Mar clock changes it will fire about an hour off from the configured local time until the cron value is updated by hand. The underlying digest job is idempotent per day regardless, so this is a timing nuisance, not a duplicate-post or correctness risk.

**Not done, on purpose.** Did not touch the laptop-based `live_runner_*.py` scripts' own `misfire_grace_time` (still APScheduler's 1-second default) -- that only matters now if ingest polling stays on a machine that sleeps, which is a separate, still-open decision. Did not add a Supabase-backed (or otherwise durable) state store for the GitHub Actions path; `actions/cache` was judged good enough for a pilot, not for a permanent production setup.

## 2026-09-23 -- misfire_grace_time fix applied to the laptop-based schedulers

**Fix.** Added `misfire_grace_time=6*3600` (6 hours) to the three `add_job()` calls diagnosed earlier the same day: the digest job in `packages/spine/src/spine/scheduling/scheduler.py`'s `build_scheduler()`, and the nudge/escalation job in both `scripts/live_runner_p1_agent_test.py` and `scripts/live_runner_teams_agent_test.py`. All three previously relied on APScheduler's own 1-second default, which is what turned a 2.5-hour laptop stall into a fully skipped digest rather than a late one.

**Verified.** `py_compile` clean on all three files. `uv run pytest packages/spine/tests/test_scheduler.py`: 5/5 passed. `uv run pytest tests/unit -k "scheduler or daily_job or nudge"`: 46/46 passed. No behavioural change to any existing test, since none of them exercise a real misfire.

**Scope.** Only the two cron-triggered jobs (digest, nudge/escalation) were touched. The `_poll_ingest` IntervalTrigger job was left alone -- a missed interval tick self-heals on the next one 5 minutes later (and `coalesce=True` is already the scheduler default), so it was never the source of the actual failure and doesn't need the same tolerance.

**Not done, on purpose.** This does not make the laptop-based path unattended-reliable on its own -- the laptop still has to be awake and online at the moment the job *does* eventually run, just with a much wider window than before. The GitHub Actions path (same day's earlier entry) is the piece that removes the "laptop must be on" dependency entirely; it's on hold pending a funded LLM provider for the runner (Ollama isn't reachable from a GitHub-hosted runner).

## 2026-09-23 -- First real Bedrock call attempted (SPN-02's third provider, previously untested): valid credentials, blocked by AWS Marketplace subscription, not code

**Context.** The GitHub Actions path is on hold pending a funded LLM provider (previous entry). AWS Bedrock access keys, region, and an inference-profile ARN for `claude-sonnet-4-20250514` were provisioned as a candidate funded provider and set in `.env` (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION=us-east-2`, `BEDROCK_MODEL_ID`). `_call_bedrock()` in `packages/spine/src/spine/llm/gateway.py` has existed since SPN-02 but, per that method's own docstring, had never actually been exercised against real AWS infrastructure -- only shape-checked against the Anthropic SDK via `inspect.signature` in `tests/unit/test_llm_gateway_bedrock_call_shape.py`.

**Finding.** `LLMGateway(provider="bedrock").generate(...)` against the real credentials returned a 403 from AWS, not a 401/auth failure: `"Model access is denied due to IAM user or service role is not authorized to perform the required AWS Marketplace actions (aws-marketplace:ViewSubscriptions, aws-marketplace:Subscribe) to enable access to this model."` The access key/secret/region themselves authenticate correctly -- AWS accepted the request and evaluated IAM permissions before rejecting it -- but this IAM identity hasn't subscribed (or isn't authorized to subscribe) to the Anthropic model listing on AWS Marketplace, which Bedrock requires separately from the inference-profile ARN itself. The gateway's degrade-to-Ollama fallback fired exactly as designed: the call returned `provider="ollama"`, `degraded=True`, with the correct output ("pong"), so no request was lost -- this was a clean, observable failure, not a silent one.

**Not fixed.** This is an AWS account permissions gap, not a code defect -- fixing it means either attaching `aws-marketplace:ViewSubscriptions`/`aws-marketplace:Subscribe` (or a broader Marketplace-access policy) to this IAM user/role, or completing the model's Marketplace subscription from the AWS console under this account, both of which are AWS console admin actions on the project owner's own AWS account. `_call_bedrock()` and its fallback behavior are confirmed correct as written and need no change.

**Security note.** The AWS access key and secret used for this test were pasted in plaintext into a chat session before being placed in `.env`; recommended the project owner rotate the key (deactivate + reissue in IAM) given that exposure, independent of the Marketplace finding above.

**Not done, on purpose.** Did not wire either GitHub Actions workflow (`daily-p1-agent-test.yml`, `daily-teams-agent-test.yml`) to `LLM_PROVIDER=bedrock` -- doing so now would just reproduce this same 403 unattended, with no local Ollama fallback available on a GitHub-hosted runner (per SPN-02, the degrade path is local-Ollama-only). That wiring is still worth doing once the Marketplace subscription is resolved, but not before. `.env`'s own `LLM_PROVIDER` was left at `ollama` (unchanged) for the same reason.

**Verified.** Real network call against AWS Bedrock, not mocked; response body quoted above is AWS's own error text. `resp.provider == "ollama"`, `resp.degraded is True`, `resp.text == "pong"` confirms the fallback path renders a correct answer even when the primary provider is unavailable.

## 2026-09-23 -- GitHub Actions path cancelled (reverses the "on hold" framing of the same day's earlier two entries); back to the laptop-based live runners, misfire fix kept

**Decision.** Rather than wait on an AWS admin to resolve the Marketplace subscription gap (previous entry) or fund a direct Anthropic API key, the project owner chose to drop the GitHub Actions path entirely and go back to the laptop-based `live_runner_p1_agent_test.py`/`live_runner_teams_agent_test.py` processes (2026-09-21/22 entries) as the standing way both channels' digest/nudge/escalation jobs run. This is a scope cut, not a technical dead end -- GitHub Actions was never broken, it was blocked on a funded LLM provider reachable from a hosted runner, and that blocker is still open; the owner judged it not worth pursuing further right now.

**What was removed.** Four files created solely for the GitHub Actions path, all untracked (never committed, so no history lost): `.github/workflows/daily-p1-agent-test.yml`, `.github/workflows/daily-teams-agent-test.yml`, `scripts/ci_refresh_graph_token.py` (the unattended Graph-token-refresh wrapper), and `scripts/run_live_nudge_teams_agent_test.py` (the one-shot nudge script that existed only because a GitHub Actions job has no equivalent of a persistent process calling `run_nudge_job()` in-line).

**What was kept.** The `misfire_grace_time=6*3600` fix (same day, earlier entry) stays in all three places it was applied -- `packages/spine/src/spine/scheduling/scheduler.py`'s `build_scheduler()`, and the nudge/escalation job in both `live_runner_p1_agent_test.py` and `live_runner_teams_agent_test.py`. That fix addresses a real laptop-sleep failure mode (the 2026-09-23 missed p1-agent-test digest) independent of which execution path (laptop vs. GitHub Actions) ends up running the job, so it remains correct and needed now that the laptop path is the only path again. `.env`'s `LLM_PROVIDER` was never changed from `ollama` during the Bedrock investigation, so nothing to revert there.

**Not done, on purpose.** Did not remove the AWS Bedrock credentials (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`, `BEDROCK_MODEL_ID`) from `.env`, or the `_call_bedrock()` provider code itself -- neither is broken, and removing working, tested code/config that simply isn't wired to anything right now is a separate judgment call the project owner didn't ask for. The Marketplace-subscription blocker documented in the previous entry, and the earlier laptop-sleep reliability problem the misfire fix addresses, both remain accurately described as still-open/still-mitigated respectively -- this entry only changes which execution path is in use, not the facts of either prior entry.

**Verified.** `git status --porcelain` after removal shows only the pre-existing tracked-file modifications (`scheduler.py`, both `live_runner_*.py` files, plus the unrelated CHN-02/CHN-25 owner-fields changes from the same day) and the unrelated demo-channel untracked files (`config/channels/demo-alpha.yaml`, `demo-beta.yaml`, `scripts/live_runner_demo_channels.py`) -- no GitHub Actions artifacts remain.

## 2026-09-23 -- p1-agent-test's Himanshu/Esandu roster labels were swapped since 2026-09-21 (corrects that day's own entry, above)

**Context.** Live-verifying CHN-08's rule engine against real `p1-agent-test` message history (same day, earlier entries) surfaced a live digest for 2026-09-22 that credited an update to nobody visibly, while `442e9b43-e50a-446c-9c4b-e612e9036bd8` -- this file's own roster comment calls that id "Esandu Obadaarachchi" -- showed `no message posted` for that day. The project owner supplied a real Teams screenshot: both of that day's real messages ("watching videos provided by warren...", "waiting for AWS Environment setup...") are shown, in the real Teams client, posted by Esandu Obadaarachchi. The message actually stored under that content in `data/p1_live.db` has `author_id=230d27f7-dd11-407c-8822-8947f0068611` -- the id this file's roster comment has called "Himanshu Ranjan" since 2026-09-21.

**Finding.** The two ids were mislabeled from the moment they were added (2026-09-21 entry, above, states the pairing as fact without a display-name-resolving Graph call to back it -- `ChannelMember.Read.All` was never granted, so there was never a way to check). The real pairing, per the project owner's direct confirmation against the live Teams screenshot:
- `230d27f7-dd11-407c-8822-8947f0068611` = **Esandu Obadaarachchi** (previously labeled Himanshu)
- `442e9b43-e50a-446c-9c4b-e612e9036bd8` = **Himanshu Ranjan** (previously labeled Esandu)

A direct `GET /users/{id}` for both ids was attempted live against the real tenant, hoping to confirm this from Graph itself rather than the screenshot alone -- both returned `403 Authorization_RequestDenied`, confirming (not just repeating) the documented `ChannelMember.Read.All` gap; Graph cannot independently corroborate this correction yet.

**Real impact, precisely bounded.** Every real digest's Participation section prints the raw AAD id, never a resolved display name (`members.display_name` is a placeholder -- the id itself -- until `ChannelMember.Read.All` lands, per the 2026-09-21 entry above). So no real Teams viewer has ever seen an incorrectly *named* person in a digest, because no digest has ever shown a name at all -- this mislabeling only ever misled humans (and this session) reading the YAML comments directly. The classification, grounding, and digest content for 2026-09-22 were all already correct; only the human-facing comment attribution was wrong.

**Fix.** Swapped the two roster comments in `config/channels/p1-agent-test.yaml` and `config/channels/demo-alpha.yaml` (which reuses these same two real ids for its own live demo roster, with the same swapped labels and demo-behavior comments describing "Himanshu... nudge gets REJECTED" / "Esandu... nudge gets APPROVED" -- also swapped to match). The roster `channel_id` values themselves were never wrong -- only the human-readable comments beside them; no code, schema, or stored data changed.

**Not done, on purpose.** Did not request or wire up `ChannelMember.Read.All` as part of this fix -- that closes this entire class of bug structurally (real Graph-resolved names instead of hand-typed comments) but is a separate, larger decision the project owner explicitly deferred in favor of this quick, screenshot-evidenced correction. Until that permission lands, any *future* person added to this roster carries the same unverified-labeling risk this entry just found -- worth checking by hand each time, not assumed safe now that this one instance is fixed.

**Verified.** Confirmed via the real Teams screenshot (both 2026-09-22 messages, matching timestamps and content, attributed to Esandu Obadaarachchi) cross-referenced against `data/p1_live.db`'s own `messages` table for the same message ids. Graph's own `/users/{id}` corroboration was attempted and is not yet possible (403, as above) -- this correction rests on the screenshot, not a Graph-verified source, exactly as scoped.

## 2026-09-23 -- Daily digest Participation lines now render real names, not raw AAD ids (closes the gap the roster-label fix, above, deliberately left open)

**Context.** Confirming the roster-label fix (previous entry) live, the project owner asked directly whether a real digest would now show names instead of ids. It would not: `p1.reporting.participation_rendering.render_participation_lines()` interpolated `record.member_id` directly, with no lookup at all -- unlike CHN-23's escalation messages, which already resolved a real `members.display_name` via a private `_resolve_display_name()` in `escalation_job.py` (2026-09-21 entry). Asked to close that gap ("how to replace their names with the ids, its ugly").

**A second, independent bug surfaced while wiring the fix.** `p1.storage.members_repo.apply_known_names()` (new, see below) is written to only ever upgrade a `members` row still sitting on its own auto-registration placeholder (`display_name == id`) -- and on the real live db, it updated 0 rows. Querying `members` directly showed `display_name` for both `230d27f7...` and `442e9b43...` was already non-placeholder text: `"Himanshu Ranjan"` and `"Esandu Obadaarachchi"` respectively -- the same **wrong, swapped** names the roster-label fix (previous entry) had just corrected in the YAML comments. Someone had evidently already hand-patched `members.display_name` once before, using the same incorrect Himanshu/Esandu mapping, before this session ever found the mismatch. `apply_known_names()`'s own safety guarantee (never overwrite a row a richer sync already populated) was, correctly by its own logic, refusing to touch data that looked authoritative but wasn't. Both rows were corrected directly (`UPDATE members SET display_name = ... WHERE id = ...`), a deliberate one-time override justified by the same live-screenshot evidence the previous entry used, not something `apply_known_names()` should be made to do automatically -- its placeholder-only guard stays as designed.

**Fix, three parts.**
1. **`p1.storage.members_repo`** (new module) -- `resolve_display_name(member_id, *, db_path)`, moved verbatim out of `escalation_job.py`'s private `_resolve_display_name()` so a second caller can reuse it instead of re-implementing the same lookup-with-fallback (the module's own docstring already warns against exactly this kind of drift, for wording -- now true of this lookup too). Also `load_known_names()` / `apply_known_names()`, reading the new committed `config/known_member_names.yaml` (versioned config, not a one-off SQL patch that a fresh `data/` would silently lose) and applying it to any placeholder row -- 0 rows today, for the reason above, but live for any future person whose real name is confirmed before `ChannelMember.Read.All` exists to resolve it automatically.
2. **`participation_rendering.py`** -- `render_participation_lines()`/`render_participation_section()` gained an optional `db_path` parameter and now resolve each `member_id` through `resolve_display_name()` before rendering. The module's own "no inferred reasons, no adjectives, no ranking" guarantees are untouched -- resolving *who* an id refers to is orthogonal to inferring *why* they're in a given state, which this function still never touches.
3. **`daily_summary.py`** -- threads `db_path` from `generate_daily_summary()` through `_render_digest_markdown()` into `render_participation_section()`, so the real db_path a live caller already passes is what actually gets used for the lookup, not the module's own unrelated default.
4. **`escalation_job.py`** -- its private `_resolve_display_name()` deleted; now imports and calls the shared one. No behavioural change there (same query, same fallback), just one fewer copy to drift.

**Verified.** `uv run pytest tests/unit -q`: 486 passed (7 pre-existing failures in `test_participation_rendering.py` fixed as part of this change -- they called the renderer without passing their own `db_path` fixture through, and `resolve_display_name()` also gained a defensive fallback for a `db_path` with no `members` table at all, not just a missing row, matching its own documented "never crashes over a missing name" promise). Live re-preview of 2026-09-22's real digest (`scripts/preview_daily_summary_p1_agent_test.py --day 2026-09-22`, read-only): Participation now reads `Himanshu Ranjan — no message posted` / `Sharon Silva — no message posted` instead of raw ids -- both real, both correctly the current (corrected) mapping.

**Not done, on purpose.** Did not wire `apply_known_names()` into any live script's startup path (e.g. alongside the `ChannelConfigStore().sync_to_db()` call every one-shot script already makes) -- the two people this session actually needed fixed were already corrected directly, and wiring it into five-plus scripts for a case that has not recurred yet is speculative scope the project owner did not ask for. `apply_known_names()` exists as a ready, tested, callable utility for whenever the next such case does come up.

## 2026-09-24 -- The weekly roll-up crashed live on raw member ids; it now renders real names (same root cause as the daily digest's Participation fix, 2026-09-23)

**Finding.** Preparing the recording's weekly roll-up beat, `generate_weekly_rollup()` against the real `p1-agent-test` data failed with `StructuredOutputError: no valid response after 3 attempts` -- every attempt rejected by `WeeklyNarrativeDraft`'s own "no digit" validator. The model's rejected sentences ended with a fragment like `...bd8 remained inactive`: `_render_briefing_block()` handed the model each member as a raw AAD object id (`230d27f7-...`), and the model copied ids -- which contain digits -- into its one narrative sentence. Fixtures never showed this because their ids are names (`alice`, `bob`). A separate, earlier attempt the same day failed on a plain Ollama `ReadTimeout` (the gateway's 60s per-request limit) while two different models (`gemma2:9b` here, `qwen2.5:7b` in the still-running live runners) contended for the same Ollama instance; a retry completed in 17s, so that one was transient.

**Fix.** `p1/reporting/weekly_summary.py`: `_render_participation`, `_render_recurring_blockers` and `_render_briefing_block` take an optional `name_of` resolver (default: the raw id, so existing callers and tests are unchanged); `generate_weekly_rollup()` passes `resolve_display_name()` (the shared resolver from `p1.storage.members_repo`, 2026-09-23). Names appear in the roll-up's own lines *and* in what the model is briefed with, which is what removes the digits at the source rather than loosening the validator.

**Verified.** New regression test `test_rollup_shows_and_briefs_the_model_with_real_names_not_raw_ids` (also required `_ScriptedGateway` to record prompts). `uv run pytest tests/unit -q`: 487 passed. Live: `scripts/demo_06_weekly_rollup.py` (new, read-only -- uses `generate_weekly_rollup`, not the persisting variant) produced a valid roll-up in ~17s with real names and a digit-free narrative.

**Observed, not fixed.** (1) Quoted messages in the roll-up (blockers, decisions, questions) show raw `<p>...</p>` HTML from `body_raw`; cosmetic. (2) The weekly figures count 7 "working days" because `p1-agent-test.yaml` still carries the TEMPORARY all-seven-days override, so percentages read oddly (e.g. 5 of 7); revert to Mon-Fri before any recording. (3) Live runners still classify with `qwen2.5:7b` until restarted; restarting them removes the two-model contention above.

## 2026-09-24 -- The weekly roll-up is now actually scheduled and published (it existed, with a configured time, but nothing ever ran it)

**Finding.** Asked "at what time will the weekly roll-up appear," a code search showed it never would: `weekly_digest_day`/`weekly_digest_time` sat in every channel config, but neither live runner nor `p1.publishing.scheduler` had any weekly job, and the only callers of the roll-up generator were the walkthrough, an ad hoc test script and a demo script -- all of which print or persist it. Config field present, behaviour absent; README's C9 row said "Done" on the strength of the generator and its golden case alone.

**Decision and design.** Built it as the daily digest's twin, deliberately reusing that path's safety machinery instead of inventing a second one: `p1.publishing.weekly_job.run_weekly_rollup_job` generates the roll-up, creates a SPN-08 proposal keyed `{channel}:{week_end}:weekly_publish`, and sends through `guarded_send()` exactly once (`already_published` on reruns, `rejected` never sends), reusing `JobResult` and the daily job's status vocabulary. One rule is new and deliberate: "first ever" is asked **per digest type** (`DigestStore.has_ever_published_type(channel, "weekly")`), not per channel. A channel with weeks of published daily digests still holds its first weekly roll-up for a human, because a weekly roll-up is a new kind of post ("no channel ever receives an unexpected bot post" applies per kind of post). Later weeks auto-approve under the same honest system approver id the daily digest uses.

**Scheduling.** `spine.scheduling.scheduler.build_scheduler`'s per-config job-adding loop was split out, behaviour unchanged, into `add_scheduled_jobs()` so a running scheduler that already has daily/ingest/nudge jobs can take a second schedule. `p1.publishing.scheduler.add_weekly_rollup_jobs()` / `is_weekly_due()` express the weekly clock as the same `ScheduleSpec` (one working day, `weekly_digest_day`, at `weekly_digest_time`, channel-local, 6h misfire grace like the others). Both `live_runner_p1_agent_test.py` and `live_runner_teams_agent_test.py` add it and log `[weekly] ...` results, wrapped so a failed roll-up is logged, not lost inside APScheduler. `scripts/run_live_weekly_p1_agent_test.py` (new) is the manual trigger for the two things a once-a-week schedule cannot do: sending the first weekly after a human approves it, and retrying a failed run.

**Consequence worth stating plainly.** The first weekly ever will NOT post on its own: on its first fire it creates a pending proposal (verified live: `awaiting_approval`, zero posts). A human approves it (approval dashboard or `scripts/approve_cli.py`), then runs `scripts/run_live_weekly_p1_agent_test.py` to send it; every later Friday is unattended.

**Verified.** New `tests/unit/test_weekly_job.py`, 9 tests: first-weekly held even with published daily digests (bug-injected -- swapping in the per-channel check makes it fail), rerun-before-approval never duplicates or sends, approve then send exactly once across reruns, rejection never sends, later weeks unattended, non-working week-end date skipped, `is_weekly_due` only on the configured day/time in the channel's own timezone, and the job added to a scheduler that already has other jobs with the right cron fields. `uv run pytest tests/unit -q`: 496 passed; spine: 25 passed. Live (real data, throwaway copy of the database, stub publisher): next fire 2026-09-25 16:00 +05:30; result `awaiting_approval`; 0 messages posted.

**Not done, on purpose.** `live_runner_demo_channels.py` was not given a weekly job (not asked for, and its two channels have no approved history). The running live runners only pick this up after a restart.

## 2026-09-24 -- Weekly roll-up times moved to after each channel's window and daily digest, and the schema now enforces it

**Finding.** Immediately after the weekly job was wired to the scheduler (previous entry), the project owner asked why it fires at 16:00 when Friday's digest is at 17:30. Checking `config/channels/*.yaml` confirmed it was a real misconfiguration, not a preference: four real channels had `weekly_digest_time` earlier than their own `update_window_end` -- `p1-agent-test` (window closes 17:30, weekly Fri 16:00), `Teams-agent-test` (13:00 vs Fri 11:00), `demo-alpha` and `demo-beta` (17:00 vs Fri 16:00). The weekly roll-up reads classified messages directly (`weekly_facts.py` joins `messages` to `classifications`, not to the daily digests), so it does not *need* the daily digests to exist -- the actual defect is that the week's last day is still open when it fires. Friday's participation would be judged before people had until the window's end to post (a 16:45 update would count as "no update" that day in the weekly), and the weekly would appear before Friday's own daily digest.

**Fix.** Weekly times set to 15 minutes after each channel's window end and daily digest: `p1-agent-test`, `demo-alpha`, `demo-beta` -> Fri 17:45; `Teams-agent-test` -> Fri 13:15. 17:45 rather than exactly 17:30 because at 17:30 the daily digest and the weekly would fire in the same minute, and `update_window_end` is inclusive (a message at 17:30:00 still counts). `ChannelConfig` now rejects any config whose `weekly_digest_time` is not strictly after both `update_window_end` and `daily_digest_time`, with an error that says why, so the misconfiguration cannot recur silently. The check compares times of day only (same timezone by construction); `proj-alpha`/`proj-beta`/`proj-gamma` already satisfied it and are unchanged.

**Verified.** Four new tests in `tests/unit/test_channel_config.py` (before the window, equal to the window end, before the daily digest even when after the window, and the accepted case); with the validator disabled 3 of the 4 fail, restored all pass. `uv run pytest tests/unit -q`: 500 passed. All seven committed configs load under the new rule.

**Not done, on purpose.** The live runners read their schedule at startup, so the new times take effect only after a restart. A late-run caveat is unchanged: the job's default `week_end` is "today in the channel's timezone" at execution, which is only right while a delayed run stays within the same local day (true for the 6h misfire grace at these times, but not something to rely on for a much later manual run -- pass `--week-end` explicitly).

## 2026-09-24 -- The live runners starved the digest of the local model; ticks no longer re-judge unchanged messages, the model timeout is configurable, and digest jobs retry and log

**Finding.** A `preview_digest_now.py` run at 13:09 crashed with an Ollama `ReadTimeout` at the digest-generation step (classification of the same messages had just worked). Diagnosis: Ollama itself was healthy (a trivial call took 2s when the model was free), but each live runner's 5-minute tick re-classified EVERY stored message -- `classify_and_persist` re-evaluated all of them and sent each rule-unsettled one to the model again, 31 model calls (~3.6 minutes of every 5-minute cycle) for `p1-agent-test` alone, plus the second runner's own tick. A local Ollama answers one request at a time, so anything else needing the model queued behind that batch, and the gateway gave up after a fixed 60s. This was not only a preview problem: the real 17:30 digest job calls the same model through the same gateway, so it was exposed to the same queue and a timeout would have failed it silently (`build_scheduler` ran `run_daily_digest_job` bare, with no logging or retry) -- the failure mode of the 2026-09-23 missed digest, by a different route.

**Fix, three parts.**
1. **`classify_and_persist(..., reuse_model_verdicts=False)`** (new, off by default -- one-shot scripts, the eval harness and prompt comparisons still re-judge everything). With it on, a message the MODEL already judged and that has not been edited since keeps its stored verdict; rules are still re-evaluated for every message every time (free, and a config change can newly settle one), and an edited message is re-judged. To make "edited since" decidable, `ClassificationStore.record()` now refreshes `created_at` on every re-record (before, an upsert left the original timestamp, so an edited message would have looked "edited since" forever and been re-judged every tick), and `ClassificationStore.model_verdicts()` exposes the stored model rows. Model verdicts of `noise` are never stored (existing design), so those few messages are still re-judged each tick. Both live runners and the demo runner pass `reuse_model_verdicts=True`.
2. **`LLMGateway.ollama_timeout`**: the fixed 60s request timeout is now `OLLAMA_TIMEOUT_SECONDS`, default 180 -- long enough to outlast a request that must wait its turn and a ~19s cold model load.
3. **`_run_with_retries` in both runners** wraps the daily digest job (via a new optional `job_fn` on `p1.publishing.scheduler.build_scheduler`) and the weekly job: up to 3 attempts, 2 minutes apart, every failure logged as `[digest]`/`[weekly] ... FAILED`, and a final "gave up -- re-run scripts/run_live_pipeline_*.py" line. Safe because both jobs are idempotent (a rerun cannot post twice).

**Verified.** 4 new tests in `test_detection_pipeline.py` (default still re-judges; reuse skips the model on an unchanged message; an edit is re-judged once and then not again; rules still win over a stored model verdict -- disabling the edit check makes the edit test fail), 5 in the new `test_runner_resilience.py` (retry-then-succeed, always-fails-never-raises with a loud final line, timeout default 180 and env override). `uv run pytest tests/unit -q`: 509 passed; spine: 25 passed. The 13:09 preview now completes, but note it returned in under a second because `LLMGateway` keeps an on-disk response cache keyed by request -- so that run demonstrates the crash is gone, not the behaviour under load; that is only proven once the runners are restarted onto this code.

**Not done, on purpose.** The runners already running (started 10:31) still hold the old code and must be restarted for any of this to take effect. Still open and unrelated: `approve_and_send` does not record `sent_at` for nudges/escalations.

## 2026-09-24 -- A ~3 hour network outage made the digest late AND stale; runners now wait for the internet, self-heal by catch-up, and a retried system send carries current content

**What happened.** The 17:30 Colombo digest job generated its digest and auto-approved its publish proposal, but the send failed at 17:30, 17:32 and 17:34 (the retry wrapper added earlier that day behaved as designed -- for a 4-minute window). The laptop had no internet (a restart at 18:59 died on `Failed to resolve login.microsoftonline.com`; the power log shows no sleep 13:00-19:02, so the runner was awake and unable to reach Microsoft, on a mobile-data Wi-Fi network). Two messages posted at 16:09/16:15 were not ingested until 20:49. The digest was finally posted at 20:57 by a manual run -- but with the 17:30 content: `daily_job` only refreshed a proposal's payload while it was PENDING, so an already-approved-but-unsent proposal was resent as a frozen snapshot, missing the two later updates (verified: the regenerated `digests` row contained them, the proposal payload that was actually posted did not). Classification and grounding were correct throughout.

**Fixes.**
1. **Catch-up** (`p1.publishing.catchup`, run by both runners every 5 minutes, first ~90s after start): if today's daily digest or weekly roll-up is due (past its scheduled time + a 10-minute grace, in the channel's own timezone), unsent, and not waiting on or decided by a human, run the job. Conservative by design: TODAY only (a Monday restart never posts Friday's digest); skips a PENDING proposal (including the first weekly ever -- no approval-queue spam), a REJECTED one, and anything already sent. Both jobs are idempotent. A `threading.Lock` in each runner serialises the scheduled job, its retries and the catch-up tick so two can never send the same digest.
2. **Stale content on retry**: `ProposalStore.refresh_payload(..., also_if_approved_by=...)` now also allows an approved-but-UNSENT proposal whose approver is exactly the system auto-approve id (no human vetted that wording); `daily_job` and `weekly_job` use it. A human-approved proposal is never rewritten; a sent one is never rewritten (both tested).
3. **Startup waits for the internet**: `_wait_for_token` retries every 30s on a connectivity error instead of crashing; a genuine auth failure (`GraphAuthError`) still stops startup.
4. **A failed send keeps its reason**: `guarded_send` now records `error` (exception type and message) in the `send_failed` write_log row (previously only "failed", which is why the 2026-09-22 Teams-agent-test failure could not be diagnosed).

**Verified.** 20 new tests (`test_catchup.py`: overdue/not-overdue rules, human-decision cases left alone, refresh allowed/refused per approver and status, the saved error reason; `test_runner_resilience.py`: startup network wait, auth error not retried, catch-up tick never raises). Bug-injection: making catch-up ignore proposal state fails the three human-decision tests. `uv run pytest tests/unit -q`: 529 passed; spine: 25 passed. Read-only check against the real database: catch-up decides "nothing overdue" for both channels, so a restart does not cause a surprise post.

**Honest limits.** Nothing here helps if no runner is running, or the laptop is asleep or off, for the whole span from the scheduled time until midnight local -- catch-up is same-day and needs a live process. `caffeinate -i` does not stop lid-close or on-battery sleep (the power log shows the laptop idle-sleeping at 19:02 once the crashed runner's caffeinate exited). The first weekly roll-up for a channel still waits for a human, by design. The only complete fix for laptop dependence is running the runners on an always-on machine.

## 2026-09-24 -- Two approval rules brought back to the spec: the weekly no longer needs its own first approval, and an approved nudge/escalation now counts as sent

**Context.** Asked whether approval applies only to the first digest, as the spec says, the project owner was right. The spec (`docs/MASTER_IMPLEMENTATION_PLAN.md`) names exactly three "first time" gates: CHN-17 "first publish behind approval" (then unattended), CHN-21 "the first nudge to any given person requires approval; thereafter within cap it runs unattended", CHN-23 "the first escalation for any person requires approval". Nothing in it gates the weekly roll-up separately.

**Fix 1 -- weekly roll-up (my own over-strictness, reverted).** The 2026-09-24 weekly-scheduling entry made the FIRST weekly roll-up per channel wait for a human even where the channel had long had daily digests published, on the reasoning that a weekly is "a new kind of post" (a per-type `DigestStore.has_ever_published_type`). That was stricter than the spec and was described as a design choice, not a spec requirement. `run_weekly_rollup_job` now asks the same per-channel question as the daily job, `DigestStore.has_ever_published(channel)`; `has_ever_published_type` and its constant were deleted as unused. Consequence: both real channels are past their first publish, so the weekly posts on its own (Fri 17:45 Asia/Colombo; Fri 13:15 America/New_York) with no approval and no manual command; a channel that has never published anything still holds its first post for a human. Tests: the old "held even after daily digests" test inverted to assert the opposite (auto-approved by the system approver, sent once). Live check on a throwaway copy of the real database with a stub publisher: `published -- sent`.

**Fix 2 -- an approved nudge/escalation was never recorded as sent (the "sent_at bug").** `approval.service.approve_and_send()` -- the path a human's Approve click takes -- sent the message and marked the proposal `applied`, but never wrote `nudges.sent_at` / `escalations.sent_at`; only the jobs' own unattended paths did. `NudgeStore.has_ever_been_nudged()` reads `sent_at`, so after a human approved someone's first nudge the spec's "thereafter unattended" never engaged (every later nudge to them was held again), and escalation -- which requires a prior sent nudge (`awaiting_nudge`) -- could never fire for anyone whose nudges were human-approved. Same class of bug as the digest's `mark_published()` fix in the same function (2026-09-19), never applied to nudges/escalations. Now `approve_and_send` calls `NudgeStore/EscalationStore.mark_sent(idempotency_key=proposal.idempotency_key, ...)` after a successful send (the proposal and nudge/escalation rows share one idempotency key). Tests: approved nudge -> `has_ever_been_nudged` True; rejected nudge -> stays False; approved escalation -> `has_ever_been_escalated` True; with the `mark_sent` calls removed the two positive tests fail.

**Live data repaired.** Two already-sent nudges (both Sharon's own, to herself: `p1-agent-test` 2026-09-20, `Teams-agent-test` 2026-09-21) were `applied` with a `sent` write_log row but `sent_at` NULL; their `sent_at` was backfilled from that write_log row so the database matches what actually happened.

**Consequences to be aware of.** Per the spec, once a person's first nudge is approved and sent, later nudges to them run unattended within the cap (1/day). `nudge_enabled` is still switched on for `p1-agent-test` as a testing override, and `working_days` still includes weekends, so an approved first nudge would lead to unattended nudges on those terms until those overrides are reverted. **Verified:** `uv run pytest tests/unit -q`: 532 passed; spine: 25 passed. The running live runners and the Streamlit dashboard hold the old code until restarted.

## 2026-09-24 -- A digest is now built from fresh Teams data, or not at all

**Gap.** The scheduled digest/weekly jobs read the database as the last 5-minute poll left it; nothing checked how old that was. So a message posted in the last few minutes before 17:30 could be missing, and -- the case that actually hit -- if polling had been failing (network down), the digest was built and posted from stale data with no warning, and a posted digest is final (catch-up cannot amend a digest that sent successfully). The 2026-09-24 digest went out missing two updates for this reason (plus the frozen-payload bug, fixed earlier the same day).

**Change.** Both live runners split `_poll_ingest` into `_ingest_and_classify` (pulls new messages and thread replies from Graph and classifies them, and RAISES on any failure) and `_poll_ingest` (unchanged behaviour: the 5-minute tick still never crashes the scheduler thread, logs `[ingest] SKIPPED/FAILED`, and still runs the Supabase mirror only on success). A module hook `_PRE_PUBLISH_REFRESH`, set in `main()`, calls `_ingest_and_classify` at the start of EVERY publishing attempt (daily digest and weekly roll-up, via `_run_with_retries`) and, through a new optional `before` argument on `run_missed_publishing`, once before the catch-up tick runs anything overdue. If the pull fails, the attempt fails -- the digest is NOT generated from whatever is already stored -- and the existing 3x retry then the 5-minute catch-up (same-day) take over, so a network outage now yields a late-but-complete digest instead of an on-time-but-incomplete one. An `_ingest_lock` serialises the poll tick and a pre-digest refresh so they never sync the same channel simultaneously.
