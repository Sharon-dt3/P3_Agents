...
This is `make run`'s own one-shot manual demo path, not the only way this job ever runs any more: `scripts/live_runner_p1_agent_test.py` and `scripts/live_runner_teams_agent_test.py` (2026-09-21/22) are real, long-running per-channel processes -- APScheduler `IntervalTrigger` polling Graph for new messages every 5 minutes, `CronTrigger` firing the daily digest/nudge/escalation check at each channel's own configured local time (17:30 Asia/Colombo for `p1-agent-test`, 13:00 America/New_York for `Teams-agent-test`) -- confirmed running unattended for hours against real Teams channels, including real nudge/escalation proposals firing on their own and a genuine APScheduler "run was missed by" drift message under load. See the Status table's C7 row and DECISION_LOG.md's live-runner entries. Every test, the eval harness, and CHN-24/27's own scripted runs still call `run_daily_digest_job` directly -- the live runners are additive, not a replacement path.
...
| C7 | MUST | Scheduled daily and weekly publishing | **Done** -- live-verified: `scripts/live_runner_p1_agent_test.py`/`live_runner_teams_agent_test.py` ran unattended for hours against real Teams channels on real APScheduler `IntervalTrigger`/`CronTrigger` schedules, firing ingest polls, digests, nudges and escalations on their own clock, no manual invocation | `src/p1/publishing/scheduler.py`, `src/p1/publishing/daily_job.py`, `scripts/run_daily.py`, `scripts/live_runner_p1_agent_test.py`, `scripts/live_runner_teams_agent_test.py`; GC6 in `src/p1/eval/chn18_cases.py`; `tests/unit/test_run_daily_full_flow.py`; DECISION_LOG.md's live-runner entries |
...
| C9 | SHOULD | Weekly roll-up with participation trend | **Done** -- generated, scheduled (channel-local `weekly_digest_day`/`weekly_digest_time`, via both live runners) and published through the same proposal/approval gate as the daily digest; approval follows the spec's one per-channel rule: only a channel's first publish ever waits for a human, so a channel that has already published a digest posts its weekly on its own (2026-09-24, see DECISION_LOG.md) | `src/p1/reporting/weekly_summary.py`, `src/p1/publishing/weekly_job.py`, `src/p1/publishing/scheduler.py`; GC11 in `src/p1/eval/chn20_cases.py`; `tests/unit/test_weekly_job.py` |
