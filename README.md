...
| C9 | SHOULD | Weekly roll-up with participation trend | **Done** -- generated, scheduled (channel-local `weekly_digest_day`/`weekly_digest_time`, via both live runners) and published through the same proposal/approval gate as the daily digest; the first weekly ever sent to a channel is held for a human, later weeks run unattended (2026-09-24, see DECISION_LOG.md) | `src/p1/reporting/weekly_summary.py`, `src/p1/publishing/weekly_job.py`, `src/p1/publishing/scheduler.py`; GC11 in `src/p1/eval/chn20_cases.py`; `tests/unit/test_weekly_job.py` |
